    ...
    public class VirtualEntity : MonoBehaviour
    {
        ...
        Vector3 clampedBallVelocity
        {
            get
            {
                ...
                if (GetComponent<Rigidbody>().velocity.sqrMagnitude < PongBehaviour.field.ball.sqrBallMinSpeed) { return GetComponent<Rigidbody>().velocity.normalized * PongBehaviour.field.ball.ballMinSpeed; }
        ...
        Vector3 clampedBallAngularVelocity
        {
            get
            {
                ...
                if (GetComponent<Rigidbody>().angularVelocity.sqrMagnitude < PongBehaviour.field.ball.sqrBallMinRotationSpeed) { return GetComponent<Rigidbody>().angularVelocity.normalized * PongBehaviour.field.ball.ballMinRotationSpeed; }
        ...
        Vector3 clampedDebuffVelocity
        {
            get
            {
                ...
                if(GetComponent<Rigidbody>().velocity.sqrMagnitude < PongBehaviour.field.debuffStore.debuffBurn.sqrDebuffMinSpeed) {return GetComponent<Rigidbody>().velocity.normalized * PongBehaviour.field.debuffStore.debuffBurn.debuffMinSpeed;}
                return GetComponent<Rigidbody>().velocity;            
            }
        }
        void FixedUpdate()
        {
            if (live)
            {
                ...
                else if (entityType == VirtualEntityType.Spike && !withinSpikeSpeedLimits)
                {
                    GetComponent<Rigidbody>().velocity = clampedSpikeVelocity;
                }
                else if (entityType == VirtualEntityType.Debuff && !withinDebuffSpeedLimits)
                {
