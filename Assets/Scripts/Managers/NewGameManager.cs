...
public class NewGameManager : PongManager
{
    ...
    public void StartGame()
    {
        ...
        if (mainSettings.tutorialsOn)
        {
            ...
            dm.pixySpeechBubble.pixyBubbleDied.AddListener(() => { ListenForPads(); dm.pixySpeechBubble.pixyBubbleDied.RemoveAllListeners(); });
        }
        else
        {
            ListenForPads();
    ...
    {
        //QUICK AND DIRTY RESET GAME 
        int fieldChildrenCount = fieldParent.transform.childCount;
        int virtualCamCount = cm.transform.childCount;
        cm.overlayCam.transform.position = cm.mainCam.transform.position = Vector3.zero;
        cm.overlayCam.transform.rotation = cm.mainCam.transform.rotation = Quaternion.identity;     
        for (int i = 0; i < virtualCamCount; i++)
        {
            GameObject.Destroy(cm.transform.GetChild(i).gameObject);
        }
        for (int i = 0; i < fieldChildrenCount; i++)
        {
            if (fieldParent.transform.GetChild(i).GetComponent<PolySpeechBubble>() == null)
            {
                GameObject.Destroy(fieldParent.transform.GetChild(i).gameObject);
            }
        }
        currentPhase = GamePhase.Startup;
        currentStage = Stage.StartMenu;
        previousStage = Stage.StartMenu;
        nextStage = Stage.DD;
        cm.SwapMainCameraMask(true);
        cm.overlayCam.transform.position = Camera.main.transform.position = Vector3.zero;
        cm.overlayCam.transform.rotation = Camera.main.transform.rotation = Quaternion.identity;        
        cm.mainCam.gameObject.SetActive(true);
        cm.leftPadCam.gameObject.SetActive(false);
        cm.rightPadCam.gameObject.SetActive(false);
        Time.timeScale = 1;
        am.StopAllCoroutines();
        cm.StopAllCoroutines();
        csm.StopAllCoroutines();
        lm.StopAllCoroutines();
        mm.StopAllCoroutines();
        StopAllCoroutines();
        newStageManager.StopAllCoroutines();
        vfx.StopAllCoroutines();
        vfx.KillAllLiveEffects();
        pongStartup.StopAllCoroutines();
        StopListenForPads();
        cm.virtualCams.Clear();
        um.inputSystemUI.actionsAsset.Disable();
        pongStartup.Startup();
